    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const order = await db.order.create({
        data: {
          ...
          discountAmount: isPixPayment
            ? originalPrice * input.quantity * 0.1
            : 0,
          shippingAmount: 0,
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (!userId || !orderId) {
          throw new TRPCError({
            code: "BAD_REQUEST",
            message: "User ID e Order ID são necessários para esta ação!",
          });
        }

        const orderExist = await ctx.db.order.findUnique({
          where: {
            id: orderId,
            userId: userId,
          },
        ...
        await ctx.db.$transaction(async (tx) => {
          if (orderExist.orderItems.length > 0) {
